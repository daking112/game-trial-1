...
export const IMPACTS: Record<DamageKind, ImpactProfile> = {
  seed: {
    ...
    flash: { count: 4, color: '#e8ffc0', speed: [0.4, 1.6], life: [0.1, 0.16], size: 16, gravity: 0 },
  ...
  ember: {
    ...
    flash: { count: 6, color: '#fff0b0', speed: [3, 8], life: [0.08, 0.15], size: 19, gravity: 0 },
    shake: 0.02, focus: 0.15,
  },
  jet: {
    burst: { count: 16, color: '#4fd8ff', speed: [3.5, 8.5], life: [0.22, 0.46], size: 7, gravity: 22 },
    flash: { count: 5, color: '#dffaff', speed: [0.5, 2], life: [0.09, 0.15], size: 14, gravity: 0 },
  ...
  bolt: {
    ...
    flash: { count: 3, color: '#ffffff', speed: [0.2, 0.9], life: [0.05, 0.1], size: 26, gravity: 0 },
    shake: 0.03, focus: 0,
  },
  shard: {
    ...
    flash: { count: 4, color: '#ffffff', speed: [1, 3], life: [0.06, 0.11], size: 15, gravity: 0 },
...
const SHIELD_HIT: ImpactProfile = {
  ...
  flash: { count: 5, color: '#ffffff', speed: [0.3, 1.2], life: [0.07, 0.12], size: 20, gravity: 0 },
  shake: 0.016, focus: 0,
};
...
export function playDeath(
  ...
  const d = DEATHS[tier];
  particles.burst(at, {
    count: d.core, color: '#ffffff', speed: [1.5 * d.speed, 5 * d.speed],
    life: [0.08, 0.18], size: 22 * d.size, gravity: 0,
  });
  particles.burst(at, {
    count: d.guts, color: trim, speed: [3 * d.speed, 11 * d.speed],
    life: [0.3, 0.7], size: 10 * d.size, gravity: 9,
  });
  particles.burst(at, {
    count: d.debris, color: shell, speed: [2 * d.speed, 8 * d.speed],
    life: [0.4, 0.95], size: 7.5 * d.size, gravity: 17,
  });
  if (d.ring) {
...
export function playShieldBreak(particles: Particles, at: THREE.Vector3, scale = 1) {
  ...
  particles.burst(at, {
    count: 8, color: '#ffffff', speed: [1, 4], life: [0.08, 0.16], size: 24 * scale, gravity: 0,
  });
}
...
export function playMuzzle(particles: Particles, at: THREE.Vector3, kind: DamageKind | undefined, dir: THREE.Vector3) {
  ...
  particles.burst(at, {
    count: 5, color: p.burst.color, speed: [2, 6], life: [0.08, 0.16], size: 9, gravity: 0, focus: 0.8,
  }, dir);
}
